...
tier4_control_msgs__msg__GateMode__create()
{
  ...
  tier4_control_msgs__msg__GateMode * msg = (tier4_control_msgs__msg__GateMode *)allocator.allocate(sizeof(tier4_control_msgs__msg__GateMode), allocator.state);
  if (!msg) {
    return NULL;
  }
...
tier4_control_msgs__msg__GateMode__Sequence__init(tier4_control_msgs__msg__GateMode__Sequence * array, size_t size)
{
  ...
  if (size) {
    data = (tier4_control_msgs__msg__GateMode *)allocator.zero_allocate(size, sizeof(tier4_control_msgs__msg__GateMode), allocator.state);
    if (!data) {
      return false;
    }
...
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  tier4_control_msgs__msg__GateMode__Sequence * array = (tier4_control_msgs__msg__GateMode__Sequence *)allocator.allocate(sizeof(tier4_control_msgs__msg__GateMode__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  ...
  tier4_control_msgs__msg__GateMode__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    tier4_control_msgs__msg__GateMode * data =
      (tier4_control_msgs__msg__GateMode *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!tier4_control_msgs__msg__GateMode__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          tier4_control_msgs__msg__GateMode__fini(&output->data[i]);
        }
        return false;
      }
    }
