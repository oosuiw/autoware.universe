  ...
  tier4_control_msgs__msg__IsPaused__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    tier4_control_msgs__msg__IsPaused * data =
      (tier4_control_msgs__msg__IsPaused *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!tier4_control_msgs__msg__IsPaused__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          tier4_control_msgs__msg__IsPaused__fini(&output->data[i]);
        }
        return false;
      }
    }
